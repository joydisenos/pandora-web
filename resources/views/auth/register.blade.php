                ...
                <div style="background-color: #fff; border: 1px solid #e5e7eb; padding: 2rem; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05);">
                    ...
                    <form method="POST" action="{{ route('register') }}">
                        ...
                        <div style="margin-bottom: 1rem;">
                            <label for="name" style="display: block; font-size: 0.75rem; font-weight: 500; color: #374151; margin-bottom: 0.25rem;">{{ __('Nombre') }}</label>
                            <input id="name" style="display: block; width: 100%; border: 1px solid #d1d5db; background-color: transparent; font-size: 0.875rem; padding: 0.5rem 0.75rem; outline: none; box-sizing: border-box;" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
                        </div>

                        <div style="margin-bottom: 1rem;">
                            <label for="email" style="display: block; font-size: 0.75rem; font-weight: 500; color: #374151; margin-bottom: 0.25rem;">{{ __('Email') }}</label>
                            <input id="email" style="display: block; width: 100%; border: 1px solid #d1d5db; background-color: transparent; font-size: 0.875rem; padding: 0.5rem 0.75rem; outline: none; box-sizing: border-box;" type="email" name="email" :value="old('email')" required />
                        </div>
                        ...
                        @if (Laravel\Jetstream\Jetstream::hasTermsAndPrivacyPolicyFeature())
                            <div style="margin-bottom: 1.5rem; display: flex; align-items: center;">
                                <x-jet-checkbox name="terms" id="terms" required style="border-color: #9ca3af; border-radius: 0.125rem;" />
                                <span style="margin-left: 0.5rem; font-size: 0.75rem; color: #4b5563;">
                                    {!! __('Acepto los :terms_of_service y la :privacy_policy', [
                                            'terms_of_service' => '<a target="_blank" href="'.route('terms.show').'" style="color: #D48A1F; text-decoration: underline;">'.__('Términos de Servicio').'</a>',
                                            'privacy_policy' => '<a target="_blank" href="'.route('policy.show').'" style="color: #D48A1F; text-decoration: underline;">'.__('Política de Privacidad').'</a>',
                                    ]) !!}
                                </span>
                            </div>
                        @endif
                        ...
                        <div style="margin-top: 1rem; text-align: center; font-size: 0.75rem; color: #4b5563;">
                            ¿Ya estás registrado? <a style="color: #D48A1F; font-weight: 600; text-decoration: none;" href="{{ route('login') }}">Inicia Sesión</a>
                        </div>
                    </form>
                </div>
